...
- 备注：
  ...
  - 对象类型的数据：内部 <i style="color:gray;font-weight:bold">“ 求助 ”</i> 了Vue3.0中的一个新函数—— ```reactive```函数(操作Proxy)。
  ```js
  // 定义对象类型的数据
  let job = ref({
    type:'前端工程师',
...
- 存在问题：
  ...
  methods: {
    ...
    deleteName(){
      // delete this.person.name // 删除属性时界面不会更新
      // 正确方式
...
- 两个小“坑”：
  ...
  - 监视reactive定义的响应式数据中某个属性（该属性是对象时配置deep才有意义）时：deep配置有效。
...
- watchEffect的套路是：不用指明监视哪个属性，监视的回调中用到哪个属性，那就监视哪个属性（且初始化时就执行一次）。
...
我们可以更加优雅的组织我们的代码，函数。让相关功能的代码更加有序的组织在一起。（借助hook函数）
...
- Vue3.0中对这些API做出了调整：
  ...
  - 将全局的API，即：```Vue.xxx```调整到应用实例（```app```）上
    ...
    | Vue.config.productionTip  | <strong style="color:#DD5145">移除</strong>（vue3可以分辨当前是开发环境还是生产环境） |
...
- <strong style="color:#DD5145">移除</strong>keyCode作为 v-on 的修饰符，同时也不再支持```config.keyCodes```（键盘事件的教程）

- <strong style="color:#DD5145">移除</strong>```v-on.native```修饰符（组件自定义事件的教程有讲）
